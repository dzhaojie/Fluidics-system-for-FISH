...
{
public:
    Valve (int pin, XIO &PCA9698Chip, bool &XIORefreshRequested, bool normallyOpen = false); // add an extra paramenter for selected XIO board.
    virtual ~Valve ();
...
class FlowSensorReader : public Thing
{
public:


    
    FlowSensorReader (int measurementPin,int measurementMaxValue);


    virtual ~FlowSensorReader();
